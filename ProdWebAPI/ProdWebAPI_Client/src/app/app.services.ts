...
export class ArtistService {
    getAll(): Artist[] {
        //return ARTIST;
        return ARTIST.map(p => this.clone(p));
    }

  get(id: number) : Artist {
    //return ARTIST.find(p => p.id === id);
    return this.clone(ARTIST.find(p => p.id === id));
  }

  save(artist: Artist){
    let originalArtist = ARTIST.find(p => p.id === artist.id);
    Object.assign(originalArtist, artist);
    if (originalArtist) Object.assign(originalArtist, artist);
    // saved muahahaha
  }
